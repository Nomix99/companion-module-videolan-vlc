...
class VlcInstance extends InstanceBase {
  ...
  reset_variables() {
    ...
    this.NextStatus = NO_CLIP;
    this.PrevStatus = NO_CLIP;
  }
  ...
  clearUnusedReservedClipVariables(targetReservedCount, oldLength) {
    ...
    for (
      let i = Math.max(targetReservedCount, knownClipCount) + 1;
      i <= (oldLength || 0);
      i++
    ) {
  ...
  updatePlayback(data) {
    ...
    const wasPlaying = pbStat({
      currentplid: this.NowPlaying,
      position: this.PlayStatus.position,
    });
    ...
    stateChanged =
      stateChanged ||
      this.PlayState !=
        (this.PlayState = ["stopped", "paused", "playing"].indexOf(
          pbInfo.state
        ));
    stateChanged =
      stateChanged || this.PlayRepeat != (this.PlayRepeat = !!pbInfo.repeat);
    stateChanged =
      stateChanged || this.PlayLoop != (this.PlayLoop = !!pbInfo.loop);
    stateChanged =
      stateChanged || this.PlayShuffle != (this.PlayShuffle = !!pbInfo.random);
    stateChanged =
      stateChanged || this.PlayFull != (this.PlayFull = !!pbInfo.fullscreen);
    ...
    got
      ...
      .then(async (response) => {
        ...
        if (response.statusCode == 401) {
          ...
          this.show_error({
            message:
              "Remote says: " + response.statusMessage + "\nIs this VLC?",
          });
        } else if (data[0] != "{") {
          // but is it JSON?
  ...
  async pollPlayback() {
    ...
    pollNow =
      pollNow || (this.PlayState == PLAYSTATE.PLAYING && !!this.config.hires);
